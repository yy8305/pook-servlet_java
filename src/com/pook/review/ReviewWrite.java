...
public class ReviewWrite extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("user_id") == "" || session.getAttribute("user_id") == null ){
			...
			out.println("location.href='/ReviewList';");
			out.println("</script>");
		}else {
			RequestDispatcher dispatcher = request.getRequestDispatcher("/review/write.jsp");
	...
	public void boardSave(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String query = "insert into board (gubun,title,contents,write_date) values ('review','"+title+"', '"+contents+"', SYSDATE())";
