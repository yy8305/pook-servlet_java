...
public class CommunityWrite extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("user_id") == "" || session.getAttribute("user_id") == null ){
			...
			out.println("location.href='/CommunityList';");
			out.println("</script>");
		}else {
	...
	public void boardSave(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String query = "insert into board (gubun,title,contents,write_date) values ('community','"+title+"', '"+contents+"', SYSDATE())";
